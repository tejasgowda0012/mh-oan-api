...
def _get_pest_login_uid_candidates(ctx: RunContext[FarmerContext]) -> List[str]:
    ...
    candidates: List[str] = []
    unique_id = ctx.deps.get_user_claim("unique_id")
    if unique_id not in (None, ""):
        candidates.append(str(unique_id))

    mobile = ctx.deps.get_user_claim("mobile")
    if mobile not in (None, ""):
        candidates.append(str(mobile))

    candidates.append("3")
    return list(dict.fromkeys(candidates))


def _get_pest_login_uid(ctx: RunContext[FarmerContext]) -> str:
    return _get_pest_login_uid_candidates(ctx)[0]
...
async def _authenticate_pest_service(ctx: RunContext[FarmerContext]) -> Dict[str, str]:
    timeout = settings.pest_detection_http_timeout
    last_payload: Any = {}

    for candidate_index, uid in enumerate(_get_pest_login_uid_candidates(ctx), start=1):
