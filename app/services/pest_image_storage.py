...
class PestImageStorage:
    ...
    def _ensure_bucket(self) -> None:
        ...
        try:
            ...
            if code not in _BUCKET_NOT_FOUND_CODES:
                raise PestImageStorageError("Unable to access the MinIO upload bucket") from exc

        try:
            client.create_bucket(Bucket=settings.minio_pest_upload_bucket)
        except (ClientError, BotoCoreError) as exc:
            raise PestImageStorageError("Unable to create the MinIO upload bucket") from exc
    ...
    def put_image(self, object_key: str, content: bytes, content_type: str) -> None:
        try:
            ...
            raise PestImageStorageError("Unable to store pest upload in MinIO") from exc
    ...
    def get_image(self, object_key: str) -> bytes:
        try:
            ...
            raise PestImageStorageError("Unable to retrieve pest upload from MinIO") from exc
